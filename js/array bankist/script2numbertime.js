...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const formatMovement = formatCurr(mov, acc.locale, acc.currency);
...
const startLogoutTimer = function () {
  //setting time to 5 minute
  let constTime = 300;

  const timerFun = function () {
    const min = String(Math.trunc(constTime / 60)).padStart(2, 0);
    const sec = String(constTime % 60).padStart(2, 0);

    //print after every second
...
console.log('US :', new Intl.NumberFormat('ne-NP', options).format(num));
...
const ingredients = ['potato', 'tomato'];

const momotimer = setTimeout(
  (ing1, ing2) => console.log(`here is momo with ${ing1} and ${ing2}`),
  4000,
  ...ingredients
);
